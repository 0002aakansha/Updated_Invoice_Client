...
export const userAsync = createAsyncThunk('user/login', async (user: loginType, { rejectWithValue }) => {
    try {
        ...
        return rejectWithValue(error.response.data.message || error.message || 'An unknown error has been occured, Please try again later!')
    }
})

export const fetchUserById = createAsyncThunk('user/fetch', async (id: string, { rejectWithValue }) => {
    try {
        const { data } = await admin({
            url: `/user/${id}`,
            headers: {
